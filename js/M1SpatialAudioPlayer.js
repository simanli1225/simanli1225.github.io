    ...
    if (predictions.length > 0) {
        predictions.forEach((prediction) => {
            ...
            for (let i = 0; i < TRIANGULATION.length / 3; i++) {
                ...
                // console.log(points, '指向')
                //  Draw triangle
                drawPath(ctx, points, true);
                // drawingUtils.drawConnectors(ctx, landmarks, mpFaceMesh.FACEMESH_TESSELATION, { color: '#C0C0C070',
                // lineWidth: 1 });
            }
            //draw the points
            ...
            // const pBottom = new THREE.Vector3(prediction.mesh[152][0], prediction.mesh[152][1],
            // prediction.mesh[152][2]); const pLeft = new THREE.Vector3(prediction.mesh[234][0],
            // prediction.mesh[234][1], prediction.mesh[234][2]); const pRight = new
            // THREE.Vector3(prediction.mesh[454][0], prediction.mesh[454][1], prediction.mesh[454][2]);
...
async function trackerMain() {
  ...
  `;
    ...
    ctx.translate(document.documentElement.clientWidth, -document.documentElement.clientHeight / 3);
    ctx.scale(-1, 1);
    ctx.scale(2, 2);
